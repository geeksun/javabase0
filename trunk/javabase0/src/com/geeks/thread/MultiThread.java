...
public class MultiThread {
		public static void main(String args[]) {
			...
			// 优先级范围1到10,MIN_X_PPRIORITY,MARIORITY,NORM_PAIORITY
			// 新线程继承创建她的父线程优先级,父线程通常有普通优先级即5,NORM_PRIORITY
...
/**
 * @author geeksun
 * @version 通过继承 Thread 类,并实现它的抽象方法 run()
 * 适当时候创建这一Thread子类的实例来实现多线程机制
 * 一个线程启动后（也即进入就绪状态）一旦获得CPU将自动调用它的 run()方法
 */
...
/**
 * @version 通过实现Runnable接口中的run()方法,再以这个实现了run()方法的类
 * 为参数创建Thread的线程实例		
